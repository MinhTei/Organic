...
if ($search) {
    $search = trim($search);
    // Tìm kiếm theo order code, order ID, tên khách hàng, hoặc email
    // Pattern: order_code LIKE | order_id = | customer_name LIKE | customer_email LIKE
    $where[] = "(o.order_code LIKE ? OR o.id = ? OR u.name LIKE ? OR u.email LIKE ?)";
    $params[] = "%$search%";
    $params[] = (int)$search;  // Convert to int cho o.id
    $params[] = "%$search%";
    $params[] = "%$search%";
}
